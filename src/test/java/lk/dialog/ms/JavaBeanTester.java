...
public class JavaBeanTester {
     ...
     */
    public static <T> void test(final Class<T> clazz, final Object object,
            ...
            if (getter != null && setter != null) {
                ...
                if (params.length == 1 && params[0] == returnType) {
                    ...
                    try {
                        ...
                        T bean = Objects.nonNull(object) ? (T) object : clazz.newInstance();
                        ...
                        assertEquals(String.format("Failed while testing property %s", prop.getName()), expectedValue,
                                     actualValue);

                    } catch (Exception ex) {
                        fail(String.format("An exception was thrown while testing the property %s: %s", prop.getName(),
                                           ex.toString()));
                    }
                }
            }
    ...
    private static Object buildValue(Class<?> clazz)
        ...
        if (clazz.getName().equalsIgnoreCase("java.util.Set")) {
            return new HashSet<>();
